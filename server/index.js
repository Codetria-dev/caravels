...
// --- RSS Feed Parser (bundled from scripts/rssParser.js) ---
import { parseRSS } from '../scripts/rssParser.js';
...
async function main() {
  ...
  if (isDev) {
    // In dev, Vite handles HMR; API routes are served alongside
  ...
  app.get('/api/news', async (req, res) => {
    if (!NEWS_API_KEY) {
      return res.json({ status: 'ok', totalResults: 0, articles: [], message: 'NewsAPI key not configured.' });
    }
    try {
  ...
  app.get('/api/gnews', async (req, res) => {
    if (!GNEWS_API_KEY) {
      return res.json({ status: 'ok', totalResults: 0, articles: [], message: 'GNews key not configured.' });
    }
    try {
  ...
  if (!isDev) {
    ...
    // Dev: use Vite's dev server as middleware
  ...
  app.listen(PORT, () => {
    console.log(`Caravels server running at http://localhost:${PORT} [${isDev ? 'dev' : 'prod'}]`);
  });
}
